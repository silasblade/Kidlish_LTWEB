...
header("location:control.php?q=5");
}
}
...
$desc = $_POST['desc'];
$id=uniqid();
$q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id',NOW(),'$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag')");

header("location:control.php?q=4&step=2&eid=$id&n=$total");
}
}
...
header("location:control.php?q=0");
}
}
...
{
$sn++;
header("location:userpage.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
...
header("location:userpage.php?q=result&eid=$eid");
}
else
{
header("location:userpage.php?q=result&eid=$eid");
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:userpage.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
